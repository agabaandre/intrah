...
class Aauth {
    ...
    public function create_user($email, $pass, $name='') {
        ...
        if ( strlen($pass) < 5 or strlen($pass) > $this->config_vars['max'] ){
            log_message('debug', print_r("Password invalid ", TRUE));
        ...
        if ( $this->CI->db->insert($this->config_vars['users'], $data )){
            ...
            log_message('debug', print_r("User ID is " .$user_id, TRUE));
            // set default group
            ...
            log_message('debug', print_r("No User Added ", TRUE));
            return FALSE;
        }
    }
    ...
    public function add_member($user_id, $group_par) {
        log_message('debug', print_r("Adding Member " .$user_id. "" .$group_par, TRUE));
        $group_id = $this->get_group_id($group_par);
